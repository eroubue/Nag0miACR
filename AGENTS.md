...
脚本依次完成：计算新版本号 → `dotnet build Nag0mi.csproj -c Release` → 将编译输出目录打包为 `Nag0mi.zip`（`Nag0mi/` 根结构，含 dll/deps.json/UI，排除 `*.pdb`）→ 更新 `Nag0mi.json` 的 `version`/`downloadUrl`/`sha256` → 在 `CHANGELOG.md` 顶部插入本版本条目 → 提交并推送 → 发布 GitHub Release（无 `gh` 时打印手动发布所需参数）。
